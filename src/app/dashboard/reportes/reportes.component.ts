...
getEventsForDate(date: Date) {
  return this.events.filter(event => event.date === format(date, 'yyyy-MM-dd', { locale: es })); 
}

navigate(months: number) {
  this.currentDate = months > 0 ? addMonths(this.currentDate, 1) : subMonths(this.currentDate, 1);
}
